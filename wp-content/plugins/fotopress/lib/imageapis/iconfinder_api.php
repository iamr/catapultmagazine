<?php
define( 'WPBEAUTIFY_ICONFINDER_URL', 'http://iconfinder.eu/api/v2/search.json' );

function wpbeautify_iconfinder_imagesearch( $search_term, $license='', $page=1, $results_per_page=36, $offset=0, $attribution_required=0, $imgtype=0, $only_modifiables = 0 ) {
	return 0;
	...
	// $iconfinder_username = $wpbeautify_sites['iconfinder']['details']['username'];
	// $iconfinder_key = $wpbeautify_sites['iconfinder']['details']['api_key'];
	...
	// 	$image_type = 'all';
		...
		$req_args = array(
			// 'username' => $iconfinder_username,
			// 'query' => $iconfinder_key,
			'query' => $search_term,
			'qf' => $image_type,
	...
	if ( !$offset ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return wpbeautify_iconfinder_togeneric ( json_decode( $response['body'] ), $is_hires, $offset, $results_per_page );
		}
	}
	else {
		$response = wp_remote_get( add_query_arg( $req_args, WPBEAUTIFY_EUROPEANA_URL ) );
		if ( is_wp_error( $response ) ) {
			...
			$arr1 = wpbeautify_iconfinder_togeneric ( json_decode( $response['body'] ), $is_hires, $offset, $results_per_page );
			$req_args['page']++;
			$response = wp_remote_get( add_query_arg( $req_args, WPBEAUTIFY_EUROPEANA_URL ) );
			if ( is_wp_error( $response ) ) {
				return 0;
			} else {
				$arr2 = wpbeautify_iconfinder_togeneric ( json_decode( $response['body'] ), $is_hires, $offset, $results_per_page );
...
function wpbeautify_iconfinder_togeneric( $images, $is_hires=0, $offset=0, $results_per_page=0 ) {
	$return_object = new stdClass();
	 // var_dump($images);
	 // exit(0);
	if ( !$images ) return 0;
...
function tsfpix_iconfinder_testapikey( $username, $apikey ) {
	$req_args = array(
		'username' => $username,
		'key' => $apikey,
		'search_term' => 'test',
